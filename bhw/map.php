...
<div class="page-header">
    <div>
        <h1>Beneficiary Map</h1>
        <div class="page-subtitle">Barangay New Bulatukan &middot; colored by latest distribution status</div>
    </div>
</div>
...
<div class="mt-card p-3 mb-3">
    <div class="row g-2 align-items-end">
        ...
        <div class="col-md-4 d-flex gap-3 small pt-2">
            ...
            <span><span class="badge rounded-pill" style="background:#8a8f8d">&nbsp;</span> No record</span>
        </div>
    </div>
</div>
...
function renderMarkers() {
    ...
    allFeatures.forEach(f => {
        ...
        const marker = L.circleMarker([f.latitude, f.longitude], {
            radius: 8,
            fillColor: colorMap[f.marker_color] || colorMap.gray,
            color: '#fff',
            weight: 2,
            fillOpacity: 0.9,
        }).addTo(map);
        ...
        marker.bindPopup(
            ...
            'Purok/Zone: ' + f.purok_zone + '<br>' +
            'Contact: ' + (f.contact_number || 'Not on file') + '<br>' +
            'Medicine: ' + (f.medicine || '-') + '<br>' +
            'Status: ' + (f.distribution_status || 'No distribution yet')
        );
        markers.push(marker);
    });
}

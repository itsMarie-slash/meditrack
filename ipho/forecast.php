...
// IPHO has full access to forecasting reports (per spec 4.5/role table) -
// this is the role that actually plans allocation off these numbers.
...
<div class="page-header">
    <div>
        ...
        <div class="page-subtitle">Use this to plan medicine allocation to the barangay</div>
    </div>
    <form method="post">
        <?php csrfField(); ?>
...
<div class="alert alert-light border small mb-3">
    <i class="bi bi-info-circle"></i> Forecasts recompute automatically every time a distribution is marked <strong>Dispensed</strong> by health center staff.
    Accuracy depends entirely on the distribution history already recorded in this system - treat low-history medicines with caution.
</div>
...
<table class="table table-hover align-middle mb-0">
    ...
    <?php if (!$latestForecasts): ?>
        <tr><td colspan="5" class="text-center text-muted py-4">No forecasts yet.</td></tr>
    <?php endif; ?>
        ...
        <tr class="<?= (int) $f['medicine_id'] === $selectedMedicineId ? 'table-active' : '' ?>">
            ...
            <td><strong><?= number_format((float) $f['predicted_quantity'], 2) ?></strong> <?= h($f['unit']) ?>
                <?php if ((float) $f['predicted_quantity'] > (int) $f['current_stock']): ?>
                    <span class="badge text-bg-warning ms-1">Allocation recommended</span>
                <?php endif; ?>
            </td>
...
new Chart(document.getElementById('forecastChart'), {
    ...
    data: {
        labels: labels,
        datasets: [
            { label: 'Actual Dispensed Quantity', data: actualData, borderColor: '#1a6b52', backgroundColor: 'rgba(26,107,82,0.1)', tension: 0.25 },
            { label: 'Forecasted Need', data: forecastData, borderColor: '#c99a2e', backgroundColor: 'rgba(201,154,46,0.1)', borderDash: [6, 4], tension: 0.25 }
        ]
    },
